...
memory usage predictable on 16 GB Macs. TTS supports both `local-tts` and
`local-tts-voice-design`; only one TTS model is kept loaded at a time. Loaded
models are kept warm for `MODEL_KEEP_ALIVE` and then unloaded when idle.
